            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="form-card-premium">
                    <form @submit.prevent="submit" id="cuentaForm" novalidate>
                        ...
                        <div class="form-section-premium compact">
                            ...
                            <div class="configuraciones-section">
                                ...
                                <div class="checkbox-group-horizontal">
                                    <!-- Cuenta de resultados - SOLO visible cuando NO es fondeadora -->
                                    ...
                                    <!-- Cuenta fondeadora - SOLO visible cuando NO es cuenta de resultados -->
                                    <label class="checkbox-premium" 
                            ...
                            <!-- ============================================ -->
                            <!-- CUENTA DE RESULTADOS PADRE -->
                            <!-- SOLO SE MUESTRA CUANDO NO ES CUENTA DE RESULTADOS Y NO ES FONDEADORA -->
                            <!-- ============================================ -->
                            ...
                            <!-- Resumen Jerarquía - SOLO cuando NO es cuenta de resultados y NO es fondeadora -->
                            <div class="jerarquia-resumen-premium" v-if="!form.es_cuenta_resultados && !form.fondeo_c && form.cuenta_resultados">
                        ...
                        <!-- ============================================ -->
                        <!-- BOTÓN CREAR CUENTA HIJA - SIEMPRE VISIBLE -->
                        <!-- ============================================ -->
                        <div class="crear-hija-container">
                            <div class="crear-hija-content">
                                <div class="crear-hija-info">
                                    ...
                                    <div>
                                        <span class="crear-hija-titulo">Nueva cuenta hija</span>
                                    </div>
                                </div>
                                ...
                                <div class="input-wrapper-premium input-with-icon">
                                    ...
                                    <select v-model="formHija.cuenta_resultados" 
                                        ...
                                        <option v-for="cuenta in cuentasResultadosFiltradas" 
                                                ...
                                                :value="cuenta.id_cuenta">
                                            {{ cuenta.codigo_cuenta }} - {{ cuenta.nombre_cuenta }}
                                            <span v-if="cuenta.id_cuenta === props.cuenta.id_cuenta" style="color: #10b981; font-weight: 700;"> (Esta cuenta)</span>
                                        </option>
                                    </select>
                                </div>
...
// COMPUTED
// ============================================
const cuentasResultadosFiltradas = computed(() => {
    // Primero usar las props
    const cuentas = props.cuentasResultados || [];
    if (cuentas.length > 0) {
        return cuentas;
    }
    // Si no hay en props, usar las cargadas localmente
    if (cuentasResultados.value.length > 0) {
        return cuentasResultados.value;
    }
    return [];
});
...
const getCuentaResultadosNombre = (id) => {
    if (!id) return '';
    const cuenta = cuentasResultadosFiltradas.value.find(c => c.id_cuenta === parseInt(id));
...
const toggleCuentaResultados = () => {
    ...
    if (form.es_cuenta_resultados) {
        ...
        form.nivel = props.cuenta.nivel || 1;
        form.fondeo_c = props.cuenta.fondeo_c || false;
        const originalValue = props.cuenta.cuenta_resultados;
        if (originalValue === 0 || originalValue === null || originalValue === undefined || originalValue === '0' || originalValue === '') {
            form.cuenta_resultados = null;
        } else {
            form.cuenta_resultados = parseInt(originalValue) || null;
        }
    }
...
const cargarCuentasResultados = async (empresaId = null) => {
    ...
    try {
        ...
        if (response.data.success) {
            ...
            // Si hay una cuenta_resultados seleccionada, validar que aún exista
            if (form.cuenta_resultados) {
                const existe = cuentasResultados.value.some(c => c.id_cuenta === parseInt(form.cuenta_resultados));
                if (!existe) {
                    // Si la cuenta seleccionada ya no existe, limpiar
                    form.cuenta_resultados = null;
                }
            }
...
const submit = () => {
    ...
    const dataToSend = {
        ...
        nivel: form.nivel,
        cuenta_resultados: form.cuenta_resultados || null,
        id_cuenta_madre: null,
    };

    if (form.es_cuenta_resultados) {
        dataToSend.cuenta_resultados = null;
        dataToSend.fondeo_c = false;
        dataToSend.nivel = 2;
        dataToSend.id_cuenta_madre = null;
...
onMounted(async () => {
    ...
    const cuentaResultadosValue = props.cuenta.cuenta_resultados;
    if (cuentaResultadosValue === 0 || cuentaResultadosValue === null || cuentaResultadosValue === undefined || cuentaResultadosValue === '0' || cuentaResultadosValue === '') {
        form.cuenta_resultados = null;
    } else {
        form.cuenta_resultados = parseInt(cuentaResultadosValue) || null;
    }
    
    if (!form.Naturaleza && props.cuenta.Naturaleza) {
        form.Naturaleza = props.cuenta.Naturaleza;
    }
    
    form.fondeo_c = props.cuenta.fondeo_c || false;
    form.es_cuenta_resultados = props.cuenta.es_cuenta_resultados || false;
