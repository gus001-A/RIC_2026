                ...
                <div class="form-card">
                    ...
                    <div v-if="tieneIva" class="info-iva-disponible">
                        ...
                        <div class="info-iva-disponible-content">
                            <span class="info-iva-disponible-title">Montos disponibles por tipo de IVA:</span>
                    ...
                    <!-- ============================================ -->
                    <div v-if="movimiento.tipo_poliza === 'EGRESO' && movimiento.cuenta_fondeadora_id" 
                         class="validacion-saldo"
                         :class="movimiento.saldo_suficiente ? 'saldo-suficiente' : 'saldo-insuficiente'">
                        <span class="validacion-icon">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path v-if="movimiento.saldo_suficiente" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/>
                    ...
                    <form @submit.prevent="submitAbono" id="abonoForm" novalidate>
                        <!-- ============================================ -->
                        <!-- FILA 1: PERSONA, CUENTA, CUENTA FONDEADORA, MARCADOR -->
                        <!-- ============================================ -->
                        <div class="form-row">
                        ...
                        <!-- ============================================ -->
                        <!-- FILA 2: FECHA VENCIMIENTO, FECHA PAGO, REFERENCIA -->
                        <!-- ============================================ -->
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <div class="readonly-value" :class="diasRestantesClass">
                                    ...
                                    <span v-if="movimiento.fecha_vencimiento">
                                        ...
                                        <span class="dias-restantes">
                                            ({{ diasRestantes > 0 ? diasRestantes + ' días restantes' : diasRestantes === 0 ? 'Hoy vence' : 'Vencido' }})
                                        </span>
                                    </span>
                        ...
                        <div class="form-actions">
                            ...
                            <div class="actions-right">
                                ...
                                <button type="submit" 
                                        :disabled="processing || !isFormValid || !validarSaldoFondeadora"
...
const validarSaldoFondeadora = computed(() => {
    ...
    // Si el monto del abono es 0, no hay problema
    if (montoAbono <= 0) return true;
    
    return montoAbono <= saldoDisponible;
});
...
const subtituloPagina = computed(() => {
    return `Registro de abono para la póliza ${props.movimiento.referencia || 'S/N'}`;
});
...
const ivasDisponibles = computed(() => {
    const ivasHeredados = props.movimiento?.ivas_heredados || [];
    const abonosRealizados = props.movimiento?.abonos || [];
    
    return ivasHeredados.map(iva => {
        let totalAbonadoIva = 0;
        abonosRealizados.forEach(abono => {
            if (abono.ivas && Array.isArray(abono.ivas)) {
                abono.ivas.forEach(ivaAbono => {
                    if (ivaAbono.id === iva.id) {
                        totalAbonadoIva += parseFloat(ivaAbono.monto) || 0;
                    }
                });
            }
        });
        
        const montoOriginal = parseFloat(iva.monto) || 0;
        const montoDisponible = Math.max(0, montoOriginal - totalAbonadoIva);
...
// ============================================
// SUBMIT ABONO - CON ALERTA DE ÉXITO
// ============================================
const submitAbono = () => {
    ...
    if (tieneIva.value) {
        let ivaInvalido = false;
        let mensajeIva = '';
        
        ivasSeleccionados.value.forEach(ivaId => {
            const montoIngresado = abonoForm.ivas[ivaId]?.monto || 0;
            const montoDisponible = obtenerMontoDisponibleIva(ivaId);
            
            if (montoIngresado > montoDisponible) {
                ivaInvalido = true;
                const porcentaje = getIvaPorcentaje(ivaId);
                mensajeIva = `El monto ingresado para el IVA ${porcentaje}% ($${formatNumber(montoIngresado)}) excede el disponible ($${formatNumber(montoDisponible)})`;
            }
        });
...
.info-leyenda-detalle strong {
    color: #92400e;
}

.info-leyenda-detalle.saldo-suficiente {
    background: rgba(16, 185, 129, 0.2);
    color: #065f46;
}

.info-leyenda-detalle.saldo-insuficiente {
...
.validacion-saldo .validacion-icon {
    display: flex;
    align-items: center;
    flex-shrink: 0;
}

.validacion-saldo .validacion-texto {
    font-weight: 500;
    flex: 1;
}

.validacion-saldo .saldo-detalle {
    font-weight: 400;
    opacity: 0.8;
    font-size: 0.8rem;
}

.validacion-saldo strong {
    font-weight: 700;
}
